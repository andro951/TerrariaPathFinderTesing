...
namespace TerrariaPathFinderTesing {
	internal static class TripleTesting {
		public static void Test() {
			...
			//Triple d1 = 2d;
			//$"d1 = 2d: {d1} ({d1.Significand}, {d1.Exponent})".LogSimple();

			//Triple f11 = 2f;
			//Triple f12 = 2f;
			//Triple f13 = f11 * f12;
			//$"f13 = {f11} * {f12}: {f13}".LogSimple();

			Triple testNum = Triple.TriplePow10(-1, 2);
			$"testNum = {testNum.S(2)}".Log();
			$"testNum(3) = {testNum.S(3)}".Log();

			//Triple testNum2 = Triple.TriplePow10();

			Triple problem = Triple.TriplePow10(-2, -1);
			$"problem = {problem.S(2)}".Log();
		...
		private static List<TestData_CalculateSignificandExponent> testData_CalculateSignificandExponents = new() {
			...
			new(-2, -1, 0, "-2.00e-1", "-2.000e-1", "-0.20", "-0.200"),
		};
		...
		public static void Test_CalculateSignificandExponent() {
			...
			foreach (TestData_CalculateSignificandExponent testData in testData_CalculateSignificandExponents) {
				Triple testNum = Triple.TriplePow10((double)testData.Significand, testData.Exponent);
